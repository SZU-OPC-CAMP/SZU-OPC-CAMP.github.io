import { cohort3Projects, demoDayProjects, mentors, metrics, partners, projects, resourceMap, trainingStats } from "./data-en.js";
...
function projectCard(project) {
  const githubLink = project.github ? `
    <a class="project-card__github" href="${project.github}" target="_blank" rel="noopener noreferrer" onclick="event.stopPropagation();" aria-label="View GitHub repository of ${project.title}">
  ...
  const videoBadge = hasVideo ? `
    <span class="project-card__video-badge" title="Includes demo video">
      <svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>
    </span>
  ` : "";
  return `
    <article class="project-card lift-on-hover" data-category="${project.category}" data-cohort="${project.cohort}">
      <a class="project-card__media" href="project-en.html?id=${project.id}" aria-label="View ${project.title}">
      ...
      <div class="project-card__body">
        ...
        <div class="project-card__foot">
          <a class="text-link" href="project-en.html?id=${project.id}">View Details</a>
          ${githubLink}
        </div>
      </div>
    </article>
...
function renderTrainingStats() {
  ...
  if (source) {
    source.textContent = `Source: ${trainingStats.source}. Aggregate statistics only.`;
  }
}
...
function renderProjectDetail() {
  ...
  document.title = `${project.title} | OPC Project Library`;
  function renderVideoCard(project) {
    ...
    return sources.map((src, index) => {
      const title = sources.length > 1 ? `Demo Video (${index + 1} / ${sources.length})` : "Demo Video";
  ...
  node.innerHTML = `
    <section class="detail-hero detail-hero--text-only">
      <div>
        <a class="back-link" href="projects-en.html">← Back to Projects</a>
    ...
    <section class="detail-grid detail-grid--with-poster">
      ...
      <article class="wide-card lift-on-hover">
        <h2>Resources We’re Looking For</h2>
...
function mentorCard(mentor) {
  return `
    <a class="mentor-card lift-on-hover" data-mentor-id="${mentor.id}" href="${mentor.homepage}" target="_blank" rel="noopener noreferrer" aria-label="Homepage of ${mentor.name}">
...
function demoDayCard(project) {
  ...
  if (project.poster) {
    links.push(`<a class="demo-day-link" href="${project.poster}" target="_blank">Poster</a>`);
  }
  if (project.video || (project.videos && project.videos.length > 0)) {
    links.push(`<span class="demo-day-badge">Demo Video</span>`);
  }
  return `
    <article class="demo-day-card lift-on-hover">
      <a class="demo-day-card__media" href="project-en.html?id=${project.id}" aria-label="View ${project.title}">
        <img src="${project.image}" alt="${project.title}">
      </a>
      <div class="demo-day-card__body">
        ...
        <div class="demo-day-team">Team: ${project.team.join(" / ")}</div>
      </div>
    </article>
  `;
}
...
function cohort3Item(project) {
  const team = project.team.length > 0 ? project.team.join(" / ") : "TBD";
  return `
